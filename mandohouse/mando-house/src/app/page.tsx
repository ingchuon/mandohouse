            ...
            {plans.map(plan => (
              <div key={plan.id} style={{ background: '#fff', border: plan.popular ? '2px solid #1B6B3A' : '0.5px solid #e0e0e0', borderRadius: 12, padding: '24px 20px' }}>
                ...
                <ul style={{ listStyle: 'none', padding: 0, marginBottom: 20 }}>
                  {plan.features.map(f => (
                    <li key={f} style={{ fontSize: 13, color: '#555', padding: '4px 0', borderBottom: '0.5px solid #f0f0f0', display: 'flex', gap: 8 }}>
                      <span style={{ color: '#1B6B3A', fontWeight: 600 }}>✓</span>{f}
                    </li>
                  ))}
                </ul>
                <Link href="/register" style={{ display: 'block', padding: 10, borderRadius: 8, fontSize: 14, fontWeight: 500, textAlign: 'center', textDecoration: 'none', background: plan.popular ? '#1B6B3A' : 'transparent', color: plan.popular ? '#fff' : '#1B6B3A', border: '1.5px solid #1B6B3A' }}>
                  เริ่มทดลองใช้
                </Link>
              </div>
            ))}
